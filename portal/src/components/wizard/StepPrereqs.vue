<template>
  <div class="space-y-3">
    <h3 class="font-semibold text-gray-900">System Prerequisites</h3>
    <p class="text-sm text-gray-500">Checking your environment before proceeding.</p>
    <div class="space-y-2">
      <div v-for="check in checks" :key="check.id" class="flex items-center gap-3 p-3 rounded-xl border" :class="checkBorder(check.state)">
        <div class="flex-shrink-0">
          <CheckCircleIcon v-if="check.state === 'ok'" class="w-5 h-5 text-green-500" />
          <XCircleIcon     v-else-if="check.state === 'error'" class="w-5 h-5 text-red-500" />
          <Loader2Icon     v-else class="w-5 h-5 text-gray-400 animate-spin" />
        </div>
        <div>
          <p class="text-sm font-medium text-gray-900">{{ check.label }}</p>
          <p v-if="check.detail" class="text-xs text-gray-500">{{ check.detail }}</p>
        </div>
      </div>
    </div>
    <p v-if="hasError" class="text-xs text-red-600">Fix the errors above before continuing. Some issues can be resolved by re-running after fixing Docker permissions.</p>
  </div>
</template>
