<template>
  <div class="space-y-4">
    <h3 class="font-semibold text-gray-900">Review & Build</h3>
    <p class="text-sm text-gray-500">Review your configuration below, then click Launch to build and start all services.</p>

    <div class="bg-gray-50 rounded-xl p-4 text-xs font-mono space-y-1 max-h-48 overflow-y-auto">
      <div v-for="(val, key) in displayConfig" :key="key" class="flex gap-2">
    ...
    <div v-if="logs.length > 0" class="bg-black rounded-xl p-4 max-h-64 overflow-y-auto">
      <p v-for="(l, i) in logs" :key="i" class="text-xs text-green-400 font-mono whitespace-pre-wrap">{{ l }}</p>
    </div>
  </div>
</template>
