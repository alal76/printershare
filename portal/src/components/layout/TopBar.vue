<template>
  <header class="h-14 border-b border-gray-100 bg-white flex items-center justify-between px-4 lg:px-8 flex-shrink-0">
    <h1 class="text-base font-semibold text-gray-900">{{ title ?? currentTitle }}</h1>

    <div class="flex items-center gap-2">
      <!-- Service status dots -->
      <div v-for="(svc, key) in topServices" :key="key" :title="String(key)" class="w-2 h-2 rounded-full" :class="dotClass(svc.status)" />
      <!-- Settings link (mobile) -->
      <RouterLink to="/settings" class="lg:hidden p-1.5 text-gray-500 hover:text-gray-900">
