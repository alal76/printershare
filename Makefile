...
.PHONY: help setup build pull start stop restart logs status \
        setup-rclone test-rclone install-native install-usbip \
        open-ui open-cups clean
...
build: ## Build the CUPS and Scanservjs Docker images
...
start: ## Start all containers
	docker compose up -d
	@echo ""
	@echo "  Scanner UI : http://$(SERVER_IP)/"
	@echo "  CUPS admin : http://$(SERVER_IP):631/"
